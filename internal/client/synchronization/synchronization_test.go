...
func TestSynchronizeNewLocalData(t *testing.T) {
	...
	stor.EXPECT().ChangeStatusOfEncryptedData(gomock.Any(), dataConflictID, "first data conflict encr data name", data.CHANGE).Return(true, nil)
	stor.EXPECT().ChangeStatusOfEncryptedData(gomock.Any(), dataConflictID, "second data conflict encr data name", data.CHANGE).Return(true, nil)
