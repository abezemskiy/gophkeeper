...
func TestAddPasswordPage(t *testing.T) {
	...
	passwordPage := EditTextPage(context.Background(), "some/url", nil, nil, nil)(testApp)
	...
	assert.Equal(t, "Текст", label, "First element of field must be named as Логин")
	...
	assert.Equal(t, "Изменить", saveButton.GetLabel(), "Первая кнопка должна быть 'Изменить'")
